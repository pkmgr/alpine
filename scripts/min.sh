...
RELEASE_TYPE="$(
	...
	i="${ID,,}"
	l=" ${ID_LIKE,,} "
	if [ "$i" = "rhel" ] || [ "$i" = "alpine" ] || [ "$i" = "ol" ] || [[ "$l" == *" rhel "* ]] || [[ "$l" == *" centos "* ]]; then
		echo ""
	fi
)"
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
__system_service_disable() { rc-service -e --quiet "$1" && __execute "rc-update del $1" "Disabling service: $1" || return 1; }
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
__system_service_enable vnstat && rc-service vnstat restart &>/dev/null
##################################################################################################################
...
_oci_pkgs="$(apk info 2>/dev/null | grep -E -- '^(oci|cloud|oracle)')"
...
#!/usr/bin/env sh
rc-update show apache2 >/dev/null 2>&1 && rc-service apache2 reload >/dev/null 2>&1
...
##################################################################################################################
find "/etc" "/usr" "/var" -iname '*.apk-new' -exec rm -Rf {} \; >/dev/null 2>&1
